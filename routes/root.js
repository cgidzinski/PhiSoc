...
//
            ...
            about.save();
                    ...
                    else
                    {
                        ...
                        newAbout.title=req.body.title;
            newAbout.body=req.body.body;
            newAbout.date=Date.now();
            newAbout.save();
                    }
 res.render('webcms.ejs', {
            user : req.user
...
DB.Showcase.findOne({_id:req.params.id}, function(err, showcase) {
 ...
 res.redirect("/showcase");
        ...
        app.get('/appcms', isLoggedIn,isAdmin, function(req, res) {
        res.render('appcms.ejs', {
            user : req.user
        });
    });
...
function isAdmin(req, res, next) {
    if (req.user.local.permissions.indexOf("Admin") != -1)
        return next();
    res.redirect('/');
}